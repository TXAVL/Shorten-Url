...
function check_and_install_packages() {
    local packages=("wget" "curl" "unzip") # Danh sách gói cần thiết
...
# Mở ứng dụng Free Fire Max nếu đã cài đặt
function open_app() {
    ...
    if command -v am &> /dev/null; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "Không thể mở ứng dụng Free Fire Max. Kiểm tra xem ứng dụng đã được cài đặt."
        fi
    else
        echo "Lệnh am không có sẵn. Thử dùng `adb` từ máy tính nếu có."
    fi
    echo "Nhấn phím bất kỳ để quay lại menu chính."
    read -n 1
}
...
# Hiển thị hướng dẫn với các màu sắc và bản quyền
function show_guide() {
    clear
